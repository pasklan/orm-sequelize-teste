...
class PessoaController {
  ...
  static async atualizaPessoa(req, res) {
    ...
    try {
      ...
      const dadosAlterados = await database.Pessoas.findOne({
        where: {
          id: Number(id),
        },
      });
  ...
  static async atualizaMatricula(req, res) {
    ...
    const { estudanteId, matriculaId } = req.params;
    try {
      await database.Matriculas.update(atualizaMatricula, {
        where: {
          id: Number(matriculaId),
          estudante_id: Number(estudanteId),
        },
      });
      const dadosAlterados = await database.Matriculas.findOne({
        where: {
          id: Number(matriculaId),
        },
      });
